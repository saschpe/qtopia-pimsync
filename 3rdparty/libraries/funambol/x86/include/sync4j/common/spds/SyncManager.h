...
class SyncManager {
    ...
    private:
        ...
        int loSupport;             // enable support for large objects - without it large outgoing items are not split
        ...
        class IncomingSyncItem : public SyncItem {
          public:
            IncomingSyncItem(const char* key,
        ...
        void initialize();
        int readSyncSourceDefinition(SyncSource& source);
        int commitChanges(SyncSource& source);
        ...
        int checkForServerChanges(SyncML* syncml, ArrayList &statusList);
