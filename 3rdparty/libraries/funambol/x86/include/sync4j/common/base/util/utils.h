...
char* wstrdup(const char* s, size_t len = STRINGDUP_NOLEN);
...
/**
 * Returns 1 is the given string is NULL or zero-length
 */
inline int isEmpty(const char*  s) {
 ...
 * Returns 1 is the given string is NULL or zero-length
 */
inline int isNotEmpty(const char*  s) {
    return (s && (strlen(s) > 0));
}
...
char *toMultibyte(const char *wc, const char *encoding = 0 );
char *toWideChar(const char *mb, const char *encoding = 0 );
...
inline const char *_wcc(const char *wc, const char *enc=0) {
