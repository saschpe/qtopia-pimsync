...
#define DEFAULT_DELIMITER (const char* )TEXT(" ")
...
class WString: public ArrayElement {
    public:
        ...
        WString(const char* str = 0, size_t len = npos);

        WString(const WString &sb);

        ~WString();

        WString& append(const char*);

        WString& append(unsigned long u, int sign = 1);

        WString& append(WString& s);

        WString& append(WString* str);

        WString& set(const char*);
        ...
        ArrayList &split (ArrayList &tokens,
                          const char *delimiters = DEFAULT_DELIMITER ) const;
         ...
         * @return - the WString
         */
        WString& join(ArrayList &tokens, const char *separator);
         ...
         * Class operators
         */
        WString& operator= (const char* sc) ;
...
WString operator+(const WString& x, const char *y);
